...
public class PeliculaDataStore {
	...
	static {
		...
		peliculas.put(UUID.randomUUID().toString(),new Pelicula("Camino hacia la felicidad","comedia",2018L));
	} 
	...
	public Collection<DtoPelicula> getAsDto() {
		return peliculas.values().stream()
				  				 .map(pelicula -> new DtoPelicula(pelicula.getNombre())).collect(Collectors.toList());
	}
